...
export default function TapPanel({ price, bid, onBid }: { price: number; bid: number; onBid: (n: number) => void }) {
  ...
  return (
    <aside className="flex w-full shrink-0 flex-col gap-5 border-b border-line bg-ink/40 p-4 md:w-[270px] md:border-b-0 md:border-r">
      ...
      {/* balance */}
      <div>
        <div className="mb-1 font-mono text-[10px] tracking-[0.2em] text-faint">{real ? "WALLET (ETH)" : "DEMO BALANCE"}</div>
        <div className="tabular text-[28px] font-black leading-none text-cyan neon-cyan">{funds.fmtMain}</div>
        <div className="tabular text-[11px] text-faint">{funds.fmtSub}</div>
      </div>

      {/* bid */}
      {real ? (
        <div>
          <div className="mb-2 font-mono text-[10px] tracking-[0.2em] text-faint">STAKE (FIXED)</div>
          <div className="clip flex items-center justify-between border border-magenta/50 bg-magenta/10 px-3 py-3">
            <span className="flex items-center gap-1.5 font-display text-sm font-bold text-magenta">
              <Zap size={14} /> {STAKE_ETH} ETH
            </span>
            <span className="font-mono text-[10px] text-faint">per tap</span>
          </div>
        </div>
      ) : (
        <div>
      ...
      <div className="mt-auto space-y-1.5 border-t border-line pt-3 font-sans text-[12px] leading-relaxed text-muted">
        {real ? (
          <>
            <p>
              <span className="text-magenta">›</span> Tap an offered cell to bet{" "}
