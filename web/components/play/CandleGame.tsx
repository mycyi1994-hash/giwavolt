...
const LOCK_SEC = 20; // betting locks this many seconds before close
...
type Candle = { open: number; high: number; low: number };
...
export default function CandleGame() {
  ...
  const candle = useRef<Record<number, Candle>>({ 60: { open: 63250, high: 63250, low: 63250 }, 180: { open: 63250, high: 63250, low: 63250 }, 300: { open: 63250, high: 63250, low: 63250 } });
    ...
    const id = setInterval(() => {
      ...
      for (const tf of TFS) {
        ...
        if (b !== bucket.current[tf.sec]) {
          ...
          candle.current[tf.sec] = { open: p, high: p, low: p };
  ...
  const place = (sec: number, dir: "up" | "down") => {
    ...
    if (left < LOCK_SEC * 1000) return; // locked near close
    if (stake <= 0 || balance[modeRef.current] < stake) return;
    sfx.place();
    adjust(modeRef.current, -stake);
    bet.current[sec] = { dir, stake, open: candle.current[sec].open };
    force((t) => t + 1);
  };
    ...
    <div className="flex h-full flex-col md:flex-row">
      ...
      <aside className="flex w-full shrink-0 flex-col gap-5 border-b border-line bg-ink/40 p-4 md:w-[240px] md:border-b-0 md:border-r">
        ...
        <div className="mt-auto space-y-1.5 border-t border-line pt-3 font-sans text-[12px] leading-relaxed text-muted">
          ...
          <p className="text-faint">BTC only · betting locks {LOCK_SEC}s before close.</p>
        </div>
      </aside>

      {/* arena */}
      <main className="flex min-w-0 flex-1 flex-col gap-4 overflow-y-auto p-4 md:p-5">
        <Featured tf={TFS[0]} price={price.current} c={candle.current[60]} bet={bet.current[60]} flash={flash.current[60]} now={now} stake={stake} onBet={place} />
        <div className="grid shrink-0 grid-cols-1 gap-4 sm:grid-cols-2">
          {TFS.slice(1).map((tf) => (
            <Mini key={tf.sec} tf={tf} price={price.current} c={candle.current[tf.sec]} bet={bet.current[tf.sec]} flash={flash.current[tf.sec]} now={now} onBet={place} />
          ))}
        </div>
      </main>
    </div>
...
function remaining(sec: number, now: number) {
  ...
  return { left, frac: left / ms, s: Math.ceil(left / 1000), locked: left < LOCK_SEC * 1000 };
}
...
// a single live candlestick (음/양봉)
function CandleStick({ c, close }: { c: Candle; close: number }) {
  const high = Math.max(c.high, close);
  const low = Math.min(c.low, close);
  const pad = (high - low) * 0.18 + 0.5;
  const top = high + pad;
  const bot = low - pad;
  ...
  const bodyTop = y(Math.max(c.open, close));
  const bodyH = Math.max(1.5, Math.abs(y(c.open) - y(close)));
  return (
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-full w-full" style={{ filter: `drop-shadow(0 0 10px ${col}88)` }}>
      <line x1="50" y1={y(high)} x2="50" y2={y(low)} stroke={col} strokeWidth="1.5" />
      <rect x="26" y={bodyTop} width="48" height={bodyH} fill={up ? "rgba(57,255,20,0.28)" : "rgba(255,43,214,0.28)"} stroke={col} strokeWidth="1.5" />
    </svg>
  );
}

function Featured({ tf, price, c, bet, flash, now, stake, onBet }: any) {
  const { frac, s, locked } = remaining(tf.sec, now);
  const up = price >= c.open;
  const pct = ((price - c.open) / c.open) * 100;
  const flashing = flash && now - flash.at < 1300;
  const col = flashing ? (flash.win ? "#39ff14" : "#ff2bd6") : "#ffd23f";
  return (
    <div className="panel clip relative flex min-h-[58vh] flex-1 flex-col overflow-hidden p-5 animate-glow md:p-6" style={{ borderColor: flashing ? col : "rgba(255,210,63,0.5)", boxShadow: `0 0 30px ${col}33` }}>
      ...
      {/* body: candle + side info */}
      <div className="grid min-h-0 flex-1 grid-cols-[1fr_auto] items-stretch gap-4">
        <div className="relative min-h-0">
          <CandleStick c={c} close={price} />
          <div className="absolute left-1 top-1 font-mono text-[10px] leading-relaxed text-faint">
            <div>O {c.open.toFixed(0)}</div>
            <div className="text-lime">H {Math.max(c.high, price).toFixed(0)}</div>
            <div className="text-magenta">L {Math.min(c.low, price).toFixed(0)}</div>
          </div>
        </div>
        <div className="flex w-[180px] flex-col items-center justify-center gap-3">
          <div className="tabular text-center font-display text-4xl font-black text-txt neon-cyan">${price.toLocaleString("en-US", { maximumFractionDigits: 0 })}</div>
          <div className={`tabular text-sm font-bold ${up ? "text-lime" : "text-magenta"}`}>{up ? "▲" : "▼"} {pct >= 0 ? "+" : ""}{pct.toFixed(2)}%</div>
          <div className="relative grid place-items-center">
            <Ring frac={frac} size={132} color={locked ? "#ff2bd6" : "#ffd23f"} />
...
function Mini({ tf, price, c, bet, flash, now, onBet }: any) {
  const { frac, s, locked } = remaining(tf.sec, now);
  const up = price >= c.open;
  const pct = ((price - c.open) / c.open) * 100;
  const flashing = flash && now - flash.at < 1300;
  return (
    <div className="panel clip flex items-center gap-4 p-4" style={{ borderColor: flashing ? (flash.win ? "#39ff14" : "#ff2bd6") : undefined }}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="tabular font-display text-xl font-black text-txt">${price.toLocaleString("en-US", { maximumFractionDigits: 0 })}</div>
