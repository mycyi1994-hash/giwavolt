...
export default function TapTradingPage() {
  ...
  const onBalanceDelta = useCallback(
    (d: number) => adjust(ctxRef.current.mode, d),
    [adjust]
  );
    ...
    <ConnectGate title="TAP TRADING">
      <div className="flex h-full">
        ...
        <main className="relative min-w-0 flex-1">
          {/* mode banner */}
          <div
            className={`panel clip absolute left-4 top-3 z-10 px-3 py-1.5 font-mono text-[11px] tracking-wider ${
              mode === "demo" ? "text-cyan" : "text-magenta"
            }`}
          >
            {mode === "demo" ? "◆ DEMO — play money" : "◆ REAL — on-chain USDC"}
          </div>
          ...
          <div className="panel clip absolute right-4 top-3 z-10 flex items-center gap-3 px-3 py-1.5 font-mono text-[11px]">
            ...
            <Stat
              label="P&L"
              value={(stats.profit >= 0 ? "+" : "") + usdc(stats.profit)}
              cls={stats.profit > 0 ? "text-lime" : stats.profit < 0 ? "text-magenta" : "text-txt"}
            />
          </div>
...
function ZoomBtn({ onClick, icon }: { onClick: () => void; icon: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="panel clip grid h-8 w-8 place-items-center text-muted transition hover:text-cyan"
    >
      {icon}
    </button>
  );
